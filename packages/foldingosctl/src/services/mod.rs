...
const AGENT_RESTART_ALL_ORDER: &[&str] = &[FOLDOPS_AGENT_SERVICE, FAH_SERVICE];
...
pub fn run(
    ...
    match subcommand {
        "restart" => {
            let unit = args
                .first()
                .ok_or_else(|| "missing service unit name".to_string())?;
            if args.len() > 1 {
                return Err(format!(
                    "unexpected services restart argument {:?}",
                    args[1]
                ));
            }
        ...
        "restart-all" => {
            if args.first().map(String::as_str) == Some("--apply") {
                if args.len() > 1 {
                    return Err(format!(
                        "unexpected services restart-all argument {:?}",
                        args[1]
                    ));
                }
...
pub fn restart_service(paths: &AppliancePaths, unit: &str) -> Result<serde_json::Value, String> {
    ...
    let scheduled = restart_unit(service.unit)?;
    let message = if scheduled {
        format!(
            "Scheduled restart for {}. The dashboard may reconnect briefly.",
            service.name
        )
    } else {
...
fn schedule_restart_all_services(_paths: &AppliancePaths) -> Result<serde_json::Value, String> {
    let script =
        "sleep 1; /usr/bin/foldingosctl services restart-all --apply --format json >/dev/null 2>&1";
...
fn restart_unit(unit: &str) -> Result<bool, String> {
    if restart_should_be_deferred(unit) {
        schedule_deferred_systemd_restart(unit)?;
        return Ok(true);
    }
    restart_runtime_unit(unit)?;
    Ok(false)
...
fn restart_should_be_deferred(unit: &str) -> bool {
    matches!(
        unit,
        FOLDOPS_SERVE_HTTPS_SERVICE | FOLDOPS_SUPERVISOR_SERVICE
    )
}
...
mod tests {
    ...
    fn agent_role_excludes_supervisor_only_services() {
        ...
        assert!(services
            .iter()
            .any(|service| service.unit == FOLDOPS_AGENT_SERVICE));
        assert!(services
            .iter()
            .any(|service| service.unit == AGENT_REGISTER_SERVICE));
        assert!(services.iter().all(|service| !service.supervisor_only));
        assert!(!services
            .iter()
            .any(|service| service.unit == FOLDOPS_SUPERVISOR_SERVICE));
    }
    ...
    fn supervisor_role_includes_foldops_runtime_services() {
        ...
        assert!(services
            .iter()
            .any(|service| service.unit == FOLDOPS_SERVE_HTTPS_SERVICE));
        assert!(services
            .iter()
            .any(|service| service.unit == FOLDOPS_PROVISION_SERVICE));
        assert!(services
            .iter()
            .any(|service| service.unit == FOLDOPS_AGENT_SERVICE));
        assert!(!services
    ...
    #[test]
    fn dashboard_serving_units_are_deferred_for_individual_restart() {
