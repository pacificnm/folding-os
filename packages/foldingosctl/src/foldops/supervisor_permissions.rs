...
pub fn ensure_supervisor_fleet_automation_permissions(paths: &AppliancePaths) -> Result<(), String> {
    ...
    if let Some(parent) = paths.foldops_assigned_manifest.parent() {
        ensure_group_directory(parent, 0o2775, gid)
            .map_err(|error| format!("configure foldops assignment directory permissions: {error}"))?;
    }
    ...
    for path in [
        ...
        if path.is_file() {
            ensure_group_file(&path, 0o664, gid).map_err(|error| {
                format!("configure {} permissions for acquire state: {error}", path.display())
            })?;
        }
    }
    Ok(())
...
pub fn ensure_supervisor_registry_writable(paths: &AppliancePaths) -> Result<(), String> {
    ...
    if let Some(parent) = paths.foldops_registry_releases_dir.parent() {
        ensure_group_directory(parent, 0o2775, gid)
            .map_err(|error| format!("configure foldops registry directory permissions: {error}"))?;
    }
    ...
    ensure_group_registry_index(&paths.foldops_registry_index, 0o664, gid, FOLDOPS_REGISTRY_INDEX_EMPTY)
    ...
    ensure_group_registry_index(&paths.tools_registry_index, 0o664, gid, TOOLS_REGISTRY_INDEX_EMPTY)
...
#[cfg(unix)]
fn ensure_group_registry_index(path: &Path, mode: u32, gid: u32, empty_content: &str) -> Result<(), String> {
...
#[cfg(not(unix))]
fn ensure_group_registry_index(_path: &Path, _mode: u32, _gid: u32, _empty_content: &str) -> Result<(), String> {
